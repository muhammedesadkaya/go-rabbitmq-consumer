...
func (c *client) deliver(brokerChannel *Channel, consumer *Consumer, delivery <-chan amqp.Delivery) {
	for d := range delivery {
		...
		Do(func(attempt int) (retry bool, err error) {
			...
			defer func() {
				...
				if r = recover(); r != nil {
					...
					if !retry || err == nil {
						...
						brokerChannel.rabbitChannel.Publish(consumer.errorExchangeName, "", false, false, errorPublishMessage(d.Body, c.parameters.RetryCount, err, fmt.Sprintf("[Exception Recover] %v %s\n", err, stack[:length])))
			...
			err = consumer.handleConsumer(Message{Payload: d.Body, MessageId: d.MessageId, Timestamp: d.Timestamp})
			if err != nil {
				fmt.Println(err)
			}
			d.Ack(false)
...
func (c *client) AddPublisher(exchangeName string, exchangeType ExchangeType, payloads ...interface{}) {
	...
	for _, item := range payloads {
		if reflect.TypeOf(item).Kind() != reflect.Struct {
			fmt.Println(fmt.Sprintf("%r  is not struct", reflect.TypeOf(item).Name()))
		} else {
	...
	if len(payloads) == 0 || payloads == nil {
		fmt.Println("payloads are not empty")
	}

	for _, item := range c.publisherBuilder.publishers {
		if item.exchangeName == exchangeName {
			fmt.Println(fmt.Sprintf("%s exchangename is already declared ", exchangeName))
		}
	}
